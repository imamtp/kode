...
switch ($action) {
    case "Import":
        $dbname = "rfe"; //database name
        $dbconn = pg_pconnect("host=localhost port=5432 dbname=$dbname user=imm password=imm"); //connectionstring
        if (!$dbconn) {
          echo "Can't connect.\n";
          exit;
        }
        ...
        $result = pg_query($dbconn, utf8_encode($contents));
        if (!$result) {
  ...
  case "Export":
    $dbname = "aktivadb2"; //database name
    $dbconn = pg_pconnect("host=localhost port=5432 dbname=$dbname user=imm password=imm"); //connectionstring
    if (!$dbconn) {
      echo "Can't connect.\n";
      exit;
  ...
  $reseq =  pg_query("SELECT sequence_name FROM information_schema.sequences");
  while($row = pg_fetch_row($reseq))
  {
      $seqname = $row[0];
      $reseqval =  pg_query("SELECT last_value FROM $seqname");
      $seqval = pg_fetch_row($reseqval);
      $val = $seqval[0]+1;

      $str.="CREATE SEQUENCE $seqname
              INCREMENT 1
              MINVALUE 1
