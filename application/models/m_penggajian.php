    ...
    function potongan($ide,$debug,$d,$m,$y,$tipe)
    {
        ...
        if($tipe=='potongan')
        {
            ...
            $sql = "SELECT a.*,b.namepotongan
                    from potongan a
                    join potongantype b ON a.idpotongantype = b.idpotongantype
                    where '$startdate' between startdate and enddate and idemployee=$ide and b.jenispotongan = 'Potongan' and a.display is null";
        } else {
              $sql = "SELECT a.*,b.namepotongan
                from potongan a
                join potongantype b ON a.idpotongantype = b.idpotongantype
                where sisaangsuran!=0 and '$startdate'>=startdate and idemployee=$ide and b.jenispotongan = 'Pinjaman' and a.display is null";
        }
        // echo $sql;
        $q = $this->db->query($sql);
        $d = array();
...
//                        "idprosesgaji" int8,
    ...
    function tunjangan($ide,$debug=null,$d,$m,$y,$totalgaji)
    {
        ...
        $sql = "select a.*,b.nametunj
                ...
                where ('$tgl' BETWEEN startdate and enddate) and idemployee=$ide and a.display is null";
                // echo $sql;
        $q = $this->db->query($sql);
        $d = array();
        if($q->num_rows()>0)
