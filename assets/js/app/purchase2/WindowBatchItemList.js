...
Ext.define('GridBatchItemPOListModel', {
    ...
    fields: [
        'purchase_batch_id', 'idpurchaseitem', 'idinventory', 'idunit', 'sku_no', 'invno', 'nameinventory', 'qty', 'price', 'disc', 'total', 'ratetax', 'tax', 'size', 'short_desc', 'size_measurement', 'warehouse_code', 'notes', 'deleted', 'no_batch'
    ],
    idProperty: 'id'
});
                            ...
                            handler: function() {
                                ...
                                for (i = 0; i < numbatch; i++) {
                                    var recBatch = new GridBatchItemPOListModel({
                                        ...
                                        'no_batch': win.selectedItem.data.no_batch || 'Autogenerated',
                                        'invno': '-' + (i + 1),
                                        'nameinventory': win.selectedItem.data.nameinventory,
                                        'qty': Math.floor((win.selectedItem.data.qty * 1 - win.selectedItem.data.qty_received * 1) / numbatch),
...
Ext.define(dir_sys + 'purchase2.WindowBatchItemList', {
    ...
    listeners: {
        show: function(win) {
            ...
            Ext.getCmp('qty_batchitemporeceipt').setValue(win.selectedItem.data.qty - win.selectedItem.data.qty_received);
